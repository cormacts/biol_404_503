...
p-value is less than 0.05, therefore we can the reject null hypothesis, can apply two-sample 
...
# sp_test <- wilcox.test(asv_abundance ~ description, ndata)
# print(sp_wctest)

## currently having some discrepancies with values (individual group members are seeings different values so all of this is tentative to change as we move forward with our analysis)

## Between species (sp) t-test: mean number of nitrogen fixing microbe species
sp_ttest <- t.test(asv_abundance ~ description, ndata)
print(sp_ttest)
```
...
Looks as though Macrocystis has a larger abundance of nitrogen cycling microbes, which contradicts what we initially hypothesized, however it is possible that when comparing proportion of nitrogen cycling microbes to the rest of the population, we may find different results. In addition, it is important to keep in mind that a large abundance of microbes found in both species are currently classified as "Unknown". So there may be more nitrogen cycling species within that group.
...
There is a vast difference between abundance of nitrogen cycling on the kelp blade tip and meristem. In contrast to our hypothesis, there is a higher abundance of nitrogen cycling microbes on the blade tip than the kelp meristem. There are also in general, way more microbes on the blade tip than the meristem. Again keeping in mind that there are still many microbes classified as "Unknown" and these could include nitrogen cycling microbes.
...
Same procedure below but with full Weigel 2019 dataset and taxa with unknown nitrogen-cycling traits:

Taxaplot Graph for Kelp Species:
...
Observations:
Above graph depicts the relative microbe abundance for each sample within taken from each kelp species. This graph clarifies the uncertainties we had when looking at the previous graphs. From here, we can see that samples taken from the Nereocystis kelp seem to have a higher proportion of nitrogen cycling microbes in comparison to the Macrosystis kelp. This observation supports our initial hypothesis and makes it more clear that the reason why there seemed to be higher nitrogen cycling abundance in our previous graph was simply because there were more samples


Taxaplot Graph for Blade locations:
...
```
Observations:
Above graph depicts the relative microbe abundance for each sample within taken the meristem and blade tip. Similar to the stack bar plot, the tip samples seem to exhibit higher proportions of nitrogen cycling microbes. Which is again, contrary to what we initially hypothesized where we expected the meristem samples to have more nitrogen cycling microbes.
